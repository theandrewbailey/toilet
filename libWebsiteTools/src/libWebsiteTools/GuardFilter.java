    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        try {
            if (null != imead.getValue(CERTIFICATE_NAME)) {
                ...
                filterConfig.getServletContext().setAttribute(CertUtil.CERTIFICATE_CHAIN, certs);
            } else {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (null != guardrepo.getHoneypotFirstBlockTime()) {
            if (null == req.getSession(false) || req.getSession().isNew()) {
                ...
                if (userAgent != null && SecurityRepo.matchesAny(userAgent, guardrepo.getDenyUAs())) {
                    killInHoney(request, response);
                    error.add((HttpServletRequest) request, null, "IP added to honeypot: Illegal User-Agent", null);
                    return;
                }
                if (SecurityRepo.matchesAny(req.getRequestURL(), guardrepo.getHoneyList())) {
                    killInHoney(request, response);
        ...
        if (null != req.getParameter("lang")) {
            ...
            if (null != selected && imead.getLocales().contains(selected)) {
        ...
        try {
            if (!reverseProxy(req, res)) {
                req.setAttribute(ORIGINAL_REQUEST_URL, req.getRequestURL().toString());
