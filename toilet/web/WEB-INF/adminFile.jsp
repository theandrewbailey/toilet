...
<c:if test="${null != uploadedfiles}"><p class="adminform">Upload successful:<c:forEach items="${uploadedfiles}" var="uploadedfile"><br/><a href="content/${uploadedfile.filename}" target="_blank" rel="noopener" >${uploadedfile.filename}</a></c:forEach></p></c:if>
</form>
<br/>
<form action="adminFile" method="POST" class="adminform adminContent" accept-charset="UTF-8"><p>Current Content:</p>
    ...
    <td><a href="content/${dir.key}${con.filename}" target="_blank" rel="noopener"><h:time datetime="${con.atime}" pattern="yyyy-MM-dd h:mm a" /></a></td>
