...
<main class="adminform adminHealth">
    ...
    <c:forEach items="${certPaths}" var="certPath"><details class="certpath">
        <summary>Certificate Path to ${certInfo.get(certPath.getCertificates().get(certPath.getCertificates().size()-1)).get("Subject")} (expires in ${fn:substringBefore(((certPath.getExpiration().getTime() - $_LIBWEBSITETOOLS_REQUEST_START_TIME.toInstant().toEpochMilli()) / 86400000),".")} days)</summary><c:forEach items="${certPath.getCertificates()}" var="cert">
        <details class="secondmin"><summary>Certificate: ${certInfo.get(cert).get("Subject")} (expires in ${fn:substringBefore(((cert.getNotAfter().getTime() - $_LIBWEBSITETOOLS_REQUEST_START_TIME.toInstant().toEpochMilli()) / 86400000),".")} days)</summary>
